...
int villageCard(int card, struct gameState * state, int handPos){
  	int currentPlayer = whoseTurn(state);
      ...
      //+2 Actions
      state->numActions = state->numActions + 1;	//bug: changed +2 to +1
...
int remodelCard(int card, int choice1, int choice2, int choice3, struct gameState * state, int handPos){
	int i;
  	int j;
  	int currentPlayer = whoseTurn(state);
  	int nextPlayer = currentPlayer + 1;
...
int councilRoomCard(int card, struct gameState * state, int handPos){
	...
	{
	  if ( i == currentPlayer )	//bug here: changed != to ==
	    {
	      drawCard(i, state);
	    }
	}
...
int adventurerCard(int card, struct gameState * state){
  	int currentPlayer = whoseTurn(state);
  	int nextPlayer = currentPlayer + 1;
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    {
    case adventurer:
		adventurerCard(card, state);
			
    case council_room:
    	councilRoomCard(card, state, handPos); 
    ...
    case remodel:
	  remodelCard(card, choice1, choice2, choice3, state, handPos); 

    case smithy:
	  smithyCard(card, state, handPos);
    
	case village:
	  	villageCard(card, state, handPos); 
     /* 
	  //+1 Card
      drawCard(currentPlayer, state);
